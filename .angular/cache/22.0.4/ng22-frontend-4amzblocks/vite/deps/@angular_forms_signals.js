...
function form(...args) {
  ...
  if (experimentalWebMcpTool) {
    const registerWebMcpForm2 = runInInjectionContext(injector, () => inject(REGISTER_WEBMCP_FORM, {
      optional: true
    }));
    if (registerWebMcpForm2) {
      runInInjectionContext(injector, () => registerWebMcpForm2(fieldRoot.fieldTree, {
...
function minDateError(minDate2, options) {
  return new MinDateValidationError(minDate2, options);
}
function maxError(max2, options) {
  return new MaxValidationError(max2, options);
}
function maxDateError(maxDate2, options) {
  return new MaxDateValidationError(maxDate2, options);
}
...
function maxDate(path, maxDateValue, config) {
  ...
  validate(path, (ctx) => {
    ...
    const max2 = ctx.state.metadata(MAX_MEMO)();
    if (max2 === void 0 || Number.isNaN(max2.getTime())) {
      return void 0;
    }
    if (value > max2) {
      if (config?.error) {
...
function maxLength(path, maxLength2, config) {
  const MAX_LENGTH_MEMO = metadata(path, createMetadataKey(), (ctx) => {
    ...
    return typeof maxLength2 === "number" ? maxLength2 : maxLength2(ctx);
  });
...
function minDate(path, minDateValue, config) {
  ...
  validate(path, (ctx) => {
    ...
    const min2 = ctx.state.metadata(MIN_MEMO)();
    if (min2 === void 0 || Number.isNaN(min2.getTime())) {
      return void 0;
    }
    if (value < min2) {
      if (config?.error) {
...
function validateAsync(path, opts) {
  ...
  const RESOURCE = createManagedMetadataKey((_state, params) => {
    if (opts.debounce !== void 0) {
      ...
      const wrappedParams = computed(() => chain(debouncedResource), ...ngDevMode ? [{
...
var registerWebMcpForm = async (formTree, options) => {
...
function inferSchemaFromFieldNode(node) {
  ...
  if (typeof value === "object") {
    ...
    for (const child of children) {
      ...
      if (child.required()) required2.push(key.toString());
    }
    return {
      type: "object",
      properties,
