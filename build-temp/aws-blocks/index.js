...
export const api = new ApiNamespace(scope, 'api', (context) => ({
    ...
    async createTodo(title, priority = 2) {
        ...
        await rt.publish('todos', user.username, { action: 'created', todoId });
        return todo;
    },
    ...
    async listTodos(sortBy) {
        ...
        if (sortBy) {
            ...
            return await Array.fromAsync(todos.query({ index, where: { userId: { equals: user.username } } }));
        }
        // Default: sorted by todoId (creation order)
        return await Array.fromAsync(todos.query({ where: { userId: { equals: user.username } } }));
    },
    /**
    ...
    async toggleTodo(todoId) {
        ...
        await rt.publish('todos', user.username, { action: 'updated', todoId });
        return { success: true };
    },
    ...
    async updatePriority(todoId, priority) {
        ...
        if (!todo)
            throw new Error('Todo not found');
        await todos.put({ ...todo, priority, version: todo.version + 1 }, { ifFieldEquals: { version: todo.version } });
        await rt.publish('todos', user.username, { action: 'updated', todoId });
        return { success: true };
    },
